...
// interacting with the roark API. You should not instantiate this client directly,
// and instead use the [NewClient] method instead.
type Client struct {
	Options      []option.RequestOption
	Health       *HealthService
	CallAnalysis *CallAnalysisService
}

// NewClient generates a new client with the default option read from the
// environment (ROARK_API_BEARER_TOKEN). The option passed in as arguments are
// applied after these default arguments, and all option will be passed down to the
// services and requests that this client makes.
func NewClient(opts ...option.RequestOption) (r *Client) {
	defaults := []option.RequestOption{option.WithEnvironmentProduction()}
	if o, ok := os.LookupEnv("ROARK_API_BEARER_TOKEN"); ok {
	...
	r.CallAnalysis = NewCallAnalysisService(opts...)
